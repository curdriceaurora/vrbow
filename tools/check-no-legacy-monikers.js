...
//     naming.
//   - The literal string "github.com/curdriceaurora/vrbow": the GitHub
...
//     that's a separate, larger decision (see PR discussion).
//
...
function main() {
  ...
  const EXEMPT_LINE = /github\.com\/curdriceaurora\/vrbow/i;
  ...
  const hits = [...gitGrep("vrbow"), ...gitGrep("vdp")].filter((line) => {
    ...
    if (EXEMPT_LINE.test(line)) return false;
    return true;
  });
