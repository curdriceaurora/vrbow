...
(() => {
  ...
  const searchBadgesModule = globalThis.VdpSearchBadges || (
    typeof require === "function" ? require("./search-badges.js") : null
  );
  const searchBadges = searchBadgesModule.createSearchBadges({
    createSafeStorageWrapper,
    getSiteRegistry,
    isSearchUrl,
    getSearchApolloData: () => latestSearchApolloData,
  });

  const initSearchManager = searchBadges.start;
  const cleanupSearchManager = searchBadges.stop;
  const scanSearchCards = searchBadges.scan;
  const requestSearchScan = searchBadges.requestScan;
  const pruneStaleSearchCards = searchBadges.prune;
  const hideTooltip = searchBadges.hideTooltip;
  ...
  function startObserver() {
    ...
    observer = new MutationObserver((mutations) => {
      ...
      if (isSearchUrl(location.href)) {
        if (searchBadges.__test.getSearchQueue()) {
          requestSearchScan();
        }
      } else {
        if (elapsed > 4000) {
